...
void iniciarBD(sqlite3 *db)
{
	...
	const char *sql_query = "CREATE TABLE CAMAREROS (DNI INT PRIMARY KEY NOT NULL, NOMBRE TEXT, APELLIDO TEXT, TEL INT, SALARIO FLOAT);";
  	...
  	const char *sql_query = "CREATE TABLE CUENTAS (DNI INT PRIMARY KEY NOT NULL, NOMBRE TEXT, APELLIDO TEXT, TEL INT, SALARIO FLOAT);";
  	rc = sqlite3_exec(db, sql_query, 0, 0, &err_message);
  	if (rc != SQLITE_OK ) 
  		cout << sqlite3_errmsg(db);
...
int altaCuenta (sqlite3 *db,int dni, string nombre,string apellido, int tel) 
{
	sqlite3_stmt *stmt;
